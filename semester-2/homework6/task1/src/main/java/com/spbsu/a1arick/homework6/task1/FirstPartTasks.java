...
public final class FirstPartTasks {
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
        Map<Album, List<Album>> uniqueMap = albums.collect(Collectors.groupingBy(Function.identity()));
    ...
    public static Optional<Album> minMaxRating(Stream<Album> albums) {
        return albums.map(album -> new Pair<>(album, album.getTracks().stream().mapToInt(Track::getRating).max().orElse(0)))
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.map(album -> new Pair<>(album, -album.getTracks().stream().mapToInt(Track::getRating).average().orElse(0.0)))
